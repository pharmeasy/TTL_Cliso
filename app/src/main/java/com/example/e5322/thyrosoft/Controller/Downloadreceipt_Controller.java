...
public class Downloadreceipt_Controller {
    ...
    public Downloadreceipt_Controller(Activity mActivity, TrackDetAdapter trackDetAdapter, int fromcome) {
    ...
    public Downloadreceipt_Controller(Activity mActivity, TrackDetails trackDetails) {
        this.mActivity = mActivity;
    ...
    public void downrecpcontroller(JSONObject jsonObject, RequestQueue requestQueue) {

        if (cd.isConnectingToInternet()) {
            final ProgressDialog progressDialog = GlobalClass.ShowprogressDialog(mActivity);
            String url= Api.downloadreceipt;
            Log.e(TAG, "downloadreceipt URL--->"+url);
                ...
                public void onResponse(JSONObject response) {
                    ...
                    try {
                        if (!GlobalClass.isNull(response.toString())) {
                            if (flag == 0) {
                                trackDetAdapter.receiptdownResponse(response,fromcome);
                            }else if (flag==1){
                                trackDetails.receiptdownResponse(response);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
