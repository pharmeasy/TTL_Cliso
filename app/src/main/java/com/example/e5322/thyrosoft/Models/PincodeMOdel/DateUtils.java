...
import com.example.e5322.thyrosoft.API.Logger;
...
//		System.out.println("Current Date and Time in GMT time zone: " + gmtDateFormat.format(new Date(calendar.getTimeInMillis())));
    ...
    public static boolean CheckTimingbetn(String s) {

        SimpleDateFormat sdf = new SimpleDateFormat("hh:mm a");
        String start = "07:59 AM";
        String endt = "08:01 PM";
        Date date = null;
        Date c_start = null;
        Date c_endt = null;
        String currentDate = "";
        try {
            date = sdf.parse(s);
            c_start = sdf.parse(start);
            c_endt = sdf.parse(endt);

            if(date.before(c_endt) && date.after(c_start)){
                return true;
            }else {
                return false;
            }
