...
public class CommGetLive_Controller {
    ...
    public CommGetLive_Controller(Activity mActivity, Communication_Activity communication_activity) {
    ...
    public void commgetliveController(JSONObject jsonObject, RequestQueue requestQueue) {

        if (cd.isConnectingToInternet()) {
            final ProgressDialog progressDialog = GlobalClass.ShowprogressDialog(mActivity);
            String url = Api.commGetLive;
            Log.e(TAG, "URL");
                ...
                public void onResponse(JSONObject response) {
                    ...
                    try {
                        if (!GlobalClass.isNull(response.toString())) {
                            if (flag == 0) {
                                communication_activity.getcommliveResponse(response);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
