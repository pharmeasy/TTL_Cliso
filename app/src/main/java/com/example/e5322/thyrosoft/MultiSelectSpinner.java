...
public class MultiSelectSpinner extends androidx.appcompat.widget.AppCompatSpinner implements DialogInterface.OnMultiChoiceClickListener {
            ...
            if (hasNone) {
                if (which == 0 && isChecked && mSelection.length > 1) {
                    ...
                    if (which != naPos && mSelection[naPos] && isChecked) {
                        ...
                        try {
                            if (((AlertDialog) dialog).getListView().getAdapter().getCount() > naPos) {
                                ((AlertDialog) dialog).getListView().setItemChecked(naPos, false);
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    public boolean performClick() {
        ...
        builder.setTitle("Please select Symptoms");
...
//        });
    ...
    public void selName(Activity activity, String str) {
        mActivity = activity;
        value = str;
    }
    ...
    public void hasNoneOption(boolean val, int naPosition) {
        hasNone = val;
        naPos = naPosition;
    }
    ...
    public interface OnMultipleItemsSelectedListener {
        ...
        void selectedStrings(List<String> strings, String value);
    }
}
