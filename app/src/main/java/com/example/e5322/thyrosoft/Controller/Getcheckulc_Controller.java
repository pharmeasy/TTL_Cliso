...
public class Getcheckulc_Controller {
    ...
    public Getcheckulc_Controller(Activity mActivity, ProductLisitngActivityNew productLisitngActivityNew) {
    ...
    public void getcheckulc_controller(String api_key, String checkUlcNumber, String user, RequestQueue requestQueue) {
        if (cd.isConnectingToInternet()) {
            final ProgressDialog progressDialog = GlobalClass.ShowprogressDialog(mActivity);

            String url = Api.getData + api_key + "/" + checkUlcNumber + "/" + user + "/getcheckulc";
                ...
                public void onResponse(JSONObject response) {
                    ...
                    try {
                        if (!GlobalClass.isNull(response.toString())) {
                            if (flag == 0) {
                                productLisitngActivityNew.getULCResponse(response);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
