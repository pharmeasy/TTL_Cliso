...
public class WindupControllerr {
    ...
    public WindupControllerr(Activity mActivity, Wind_up_fragment wind_up_fragment) {
    ...
    public void getwindupcontroller(String api_key, String user, String DatePassToApi, RequestQueue requestQueue) {
        if (cd.isConnectingToInternet()) {
            final ProgressDialog progressDialog = GlobalClass.ShowprogressDialog(mActivity);
            String url = Api.windupApi + "" + api_key + "/" + user + "/" + DatePassToApi + "/getwowindup";
            Log.e(TAG, "Windup URL --->" + url);
                ...
                public void onResponse(JSONObject response) {
                    ...
                    try {
                        if (!GlobalClass.isNull(response.toString())) {
                            if (flag == 0) {
                                wind_up_fragment.getwindResponse(response);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
