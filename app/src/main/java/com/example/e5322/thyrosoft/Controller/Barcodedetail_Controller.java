...
public class Barcodedetail_Controller {
    ...
    public Barcodedetail_Controller(Activity mActivity, Start_New_Woe start_new_woe) {
    ...
    public void getbarcodedetais(String api_key, String user, String passBarcode, RequestQueue requestQueue) {
        if (cd.isConnectingToInternet()) {
                ...
                public void onResponse(JSONObject response) {
                    ...
                    try {
                        if (!GlobalClass.isNull(response.toString())) {
                            if (flag == 0) {
                                start_new_woe.getbarcodedetail(response);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
            ...
            Log.e(TAG, "RecheckType: URL" + jsonObjectRequestPop);
